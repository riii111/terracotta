...
mod tests {
    ...
    #[test]
    fn reconcile_view_selects_the_first_row_only_without_a_valid_selection() {
        struct ReconcileCase {
            name: &'static str,
            review: PlanReview,
            inputs: &'static [OverviewInput],
            expected: Option<usize>,
        }
        let mut empty_plan = related_review().plan().clone();
        empty_plan.resource_changes.clear();
        ...
        for case in [
            ...
            ReconcileCase {
                name: "empty_plan",
                review: related_review().with_plan(empty_plan),
                inputs: &[],
                expected: None,
            },
            ReconcileCase {
                name: "selected_row",
                review: related_review(),
                inputs: &[OverviewInput::Down, OverviewInput::Down],
                expected: Some(1),
            },
            ReconcileCase {
                name: "filtered_rows",
                ...
                inputs: &[
                    ...
                    OverviewInput::SearchChar('n'),
                    OverviewInput::SearchChar('e'),
                    OverviewInput::SearchChar('t'),
                    OverviewInput::SearchConfirm,
                ],
                expected: Some(0),
            },
            ReconcileCase {
                name: "filter_without_matches",
                review: related_review(),
                inputs: &[
                    OverviewInput::SearchStart,
                    OverviewInput::SearchChar('z'),
                    OverviewInput::SearchConfirm,
                ],
                expected: None,
            },
        ] {
            let state = OverviewSessionState::new(case.review);
            let mut view = OverviewViewState::default();
            ...
            reconcile_view(Rect::new(0, 0, 120, 40), &state, &mut view);

            let content =
                OverviewContent::from_review(state.review(), view.filter(), view.expanded());
            assert_eq!(view.selected(), case.expected, "case: {}", case.name);
            let expected_node = case.expected.map(|index| {
                content.rows[index]
                    .node_id
                    .as_ref()
                    .expect("row relation node")
            });
            assert_eq!(
                view.selected_node_id(&content),
                expected_node,
                "case: {}",
                case.name
            );
