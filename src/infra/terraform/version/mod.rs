...
pub(crate) fn read_version_with_arguments(
    ...
    if output.interrupted {
        return Err(interrupted_error(TerraformCommand::Version, output));
    }
    if !output.status.is_some_and(ProcessStatus::is_success) {
        return Err(non_zero_error(TerraformCommand::Version, output));
    }
    let document = serde_json::from_slice::<Value>(&output.output.stdout).map_err(|error| {
        TerraformExecutionError::new(TerraformExecutionErrorKind::InvalidVersion {
