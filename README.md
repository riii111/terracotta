...
Terraform rootで、対話的な端末から実行する。

```sh
terracotta plan
```

Terracottaはその場で`terraform init`と`terraform plan -out=<一時ファイル>`を実行し、終了後にplan全文を確認できる画面を開く。planの内容と対象のworkspaceを確認してから、必要ならapplyへ進む。plan全文の閲覧・検索、機微値をマスクしたplanのclipboardコピー、applyせずに終了する操作を提供する。
